...
void CLR_RT_HeapBlock_NativeEventDispatcher::HandlerMethod_RecoverFromGC()
{
    ...
    NANOCLR_FOREACH_NODE(
        CLR_RT_HeapBlock_NativeEventDispatcher,
        event,
        CLR_RT_HeapBlock_NativeEventDispatcher::eventList)
    {
        event->RecoverFromGC();
    }
...
HRESULT CLR_RT_HeapBlock_NativeEventDispatcher::ExtractInstance(
...
HRESULT CLR_RT_HeapBlock_NativeEventDispatcher::StartDispatch(
    CLR_RT_ApplicationInterrupt *appInterrupt,
    CLR_RT_Thread *th)
{
    NATIVE_PROFILE_CLR_IOPORT();
    ...
    dlg = event[Library_nf_rt_events_native_nanoFramework_Runtime_Events_NativeEventDispatcher::FIELD__threadSpawn]
              .DereferenceDelegate();
    FAULT_ON_NULL(dlg);
...
void CLR_RT_HeapBlock_NativeEventDispatcher::ThreadTerminationCallback(void *arg)
{
    ...
    FreeManagedEvent(
        (interrupt.data1 >> 8) & 0xff, // category
